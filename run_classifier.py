...
tf.flags.DEFINE_string("predict_dir", "",
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
            ...
            output_info = FLAGS.output_dir + "output_info.txt"
            with open(output_info,'a',encoding = "utf-8") as outFile:
        ...
        for key, val in sorted(eval_results[0].items(), key=lambda x: x[0]):
            log_str += "{} {} | ".format(key, val)
        tf.logging.info(log_str)
        eval_output_file = FLAGS.output_dir + "eval_info.txt"
        with open(eval_output_file,'a',encoding="utf-8") as eval_output:
            eval_output.write(log_str)
